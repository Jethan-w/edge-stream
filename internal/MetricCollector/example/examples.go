...
func ExampleBasicUsage() {
	...
	// 注册Gauge指标（瞬时值）
	_, err := collector.RegisterGauge("jvm.memory.used", func() float64 {
		// 模拟JVM内存使用量
...
func ExampleMetricStorage() {
	...
	// 创建指标快照
	snapshot := &MetricCollector.MetricsSnapshot{
		Timestamp: time.Now(),
		...
		Histograms: map[string]MetricCollector.HistogramData{
			"processing.time": {
				Count: 100,
				Min:   10.0,
				Max:   200.0,
		...
		Timers: map[string]MetricCollector.TimerData{
			"request.duration": {
				Count: 50,
		...
		Meters: map[string]MetricCollector.MeterData{
			"requests.per.second": {
				Count:             1000,
				MeanRate:          10.5,
				OneMinuteRate:     10.0,
...
func (dcpmp *DatabaseConnectionPoolMetricProvider) RegisterMetrics(collector MetricCollector.MetricCollector) {
	// 注册活跃连接数指标
...
func (mpr *MockProcessorRegistry) GetAllProcessors() ([]MetricCollector.Processor, error) {
	var processors []MetricCollector.Processor
...
func (mpgr *MockProcessGroupRegistry) GetAllGroups() ([]MetricCollector.ProcessGroup, error) {
	var groups []MetricCollector.ProcessGroup
...
func (mcm *MockClusterManager) GetAllNodes() ([]MetricCollector.NiFiNode, error) {
	var nodes []MetricCollector.NiFiNode
