...
func (scm *StateCheckpointManager) CreateCheckpoint(componentID string, stateMap *StateMap) error {
	scm.mu.Lock()
	defer scm.mu.Unlock()
...
func (scm *StateCheckpointManager) RestoreLatestCheckpoint(componentID string) (*StateMap, error) {
	scm.mu.RLock()
...
type StateUpdateRecord struct {
	Timestamp     int64     `json:"timestamp"`
	ComponentID   string    `json:"component_id"`
	PreviousState *StateMap `json:"previous_state,omitempty"`
	NewState      *StateMap `json:"new_state,omitempty"`
...
func (walm *WriteAheadLogManager) LogStateUpdate(componentID string, previousState, newState *StateMap) error {
	walm.mu.Lock()
	defer walm.mu.Unlock()
