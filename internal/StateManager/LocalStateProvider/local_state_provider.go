...
// Initialize 初始化提供者
func (fsp *FileSystemStateProvider) Initialize(config statemanager.ProviderConfiguration) error {
	fsp.mu.Lock()
	defer fsp.mu.Unlock()
...
// LoadState 加载组件状态
func (fsp *FileSystemStateProvider) LoadState(componentID string) (*statemanager.StateMap, error) {
	fsp.mu.RLock()
	...
	if _, err := os.Stat(componentStatePath); os.IsNotExist(err) {
		// 文件不存在，返回空状态
		return statemanager.NewStateMap(nil, statemanager.StateScopeLocal), nil
	}
	...
	// 尝试解析为 JSON 格式
	var stateMap statemanager.StateMap
...
func (fsp *FileSystemStateProvider) PersistState(componentID string, stateMap *statemanager.StateMap) error {
	fsp.mu.Lock()
	defer fsp.mu.Unlock()
...
type MemoryStateProvider struct {
	states map[string]*statemanager.StateMap
	mu     sync.RWMutex
}
...
func NewMemoryStateProvider() *MemoryStateProvider {
	return &MemoryStateProvider{
		states: make(map[string]*statemanager.StateMap),
	}
}

// Initialize 初始化提供者
func (msp *MemoryStateProvider) Initialize(config statemanager.ProviderConfiguration) error {
	// 内存提供者不需要特殊初始化
	return nil
}

// LoadState 加载组件状态
func (msp *MemoryStateProvider) LoadState(componentID string) (*statemanager.StateMap, error) {
	msp.mu.RLock()
...
// PersistState 持久化组件状态
func (msp *MemoryStateProvider) PersistState(componentID string, stateMap *statemanager.StateMap) error {
	msp.mu.Lock()
	defer msp.mu.Unlock()

	// 创建状态的副本
	stateCopy := &statemanager.StateMap{
		StateValues: make(map[string]string),
...
func (msp *MemoryStateProvider) Shutdown() error {
	...
	// 清空内存中的状态
	msp.states = make(map[string]*statemanager.StateMap)
	return nil
}
