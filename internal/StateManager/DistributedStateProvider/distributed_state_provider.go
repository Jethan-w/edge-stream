package distributedstateprovider
...
// Initialize 初始化提供者
func (zsp *ZooKeeperStateProvider) Initialize(config statemanager.ProviderConfiguration) error {
	zsp.mu.Lock()
	defer zsp.mu.Unlock()
...
// LoadState 加载组件状态
func (zsp *ZooKeeperStateProvider) LoadState(componentID string) (*statemanager.StateMap, error) {
	zsp.mu.RLock()
	...
	if err != nil {
		// 节点不存在，返回空状态
		return statemanager.NewStateMap(nil, statemanager.StateScopeCluster), nil
	}
...
func (zsp *ZooKeeperStateProvider) PersistState(componentID string, stateMap *statemanager.StateMap) error {
	zsp.mu.Lock()
	defer zsp.mu.Unlock()
...
func NewStateConsistencyManager(localProvider, clusterProvider statemanager.StateProvider) *StateConsistencyManager {
...
func (scm *StateConsistencyManager) selectLatestState(local, cluster *statemanager.StateMap) *statemanager.StateMap {
