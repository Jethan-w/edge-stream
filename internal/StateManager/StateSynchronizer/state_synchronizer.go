...
func (spr *StateProviderRegistry) RegisterProvider(providerName string, providerType reflect.Type) error {
	...
	if !providerType.Implements(reflect.TypeOf((*statemanager.StateProvider)(nil)).Elem()) {
...
func (spr *StateProviderRegistry) CreateProvider(providerName string, config statemanager.ProviderConfiguration) (statemanager.StateProvider, error) {
	spr.mu.RLock()
	...
	provider := providerValue.Interface().(statemanager.StateProvider)
...
type StateProviderExtension interface {
	...
	CreateProvider() statemanager.StateProvider
}

// StateSynchronizer 状态同步器
type StateSynchronizer struct {
	registry        *StateProviderRegistry
	localProvider   statemanager.StateProvider
	clusterProvider statemanager.StateProvider
...
type SyncConfiguration struct {
	LocalProviderConfig   statemanager.ProviderConfiguration
	ClusterProviderConfig statemanager.ProviderConfiguration
	SyncInterval          time.Duration
	AutoSync              bool
}
...
func (ss *StateSynchronizer) GetLocalProvider() statemanager.StateProvider {
	ss.mu.RLock()
	defer ss.mu.RUnlock()
...
func (ss *StateSynchronizer) GetClusterProvider() statemanager.StateProvider {
	ss.mu.RLock()
	defer ss.mu.RUnlock()
...
func (ss *StateSynchronizer) selectLatestState(local, cluster *statemanager.StateMap) *statemanager.StateMap {
...
// Initialize 初始化提供者
func (rsp *RedisStateProvider) Initialize(config statemanager.ProviderConfiguration) error {
	rsp.mu.Lock()
	defer rsp.mu.Unlock()
...
// LoadState 加载组件状态
func (rsp *RedisStateProvider) LoadState(componentID string) (*statemanager.StateMap, error) {
	rsp.mu.RLock()
	...
	return statemanager.NewStateMap(stateValues, statemanager.StateScopeCluster), nil
}

// PersistState 持久化组件状态
func (rsp *RedisStateProvider) PersistState(componentID string, stateMap *statemanager.StateMap) error {
	rsp.mu.Lock()
	defer rsp.mu.Unlock()
...
func (rse *RedisStateProviderExtension) CreateProvider() statemanager.StateProvider {
