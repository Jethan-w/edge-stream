...
// Initialize 初始化重试管理器
func (r *RetryManager) Initialize(config *sink.ReliabilityConfig) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	case sink.ReliabilityStrategyAtMostOnce:
		r.strategy = &FixedDelayStrategy{
			delay:      config.BaseRetryDelay,
			maxRetries: 0, // 不重试
		}
	case sink.ReliabilityStrategyAtLeastOnce:
	...
	case sink.ReliabilityStrategyExactlyOnce:
		r.strategy = &ExponentialBackoffStrategy{
			baseDelay:  config.BaseRetryDelay,
			maxDelay:   config.MaxRetryDelay,
			multiplier: config.RetryMultiplier,
