...
type SecureTransferHandler struct {
	...
	// 配置
	config *sink.SecurityConfiguration
...
func (s *SecureTransferHandler) Initialize(config *sink.SecurityConfiguration) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	// 初始化 OAuth2 客户端
	if config.AuthenticationMethod == sink.AuthenticationMethodOAuth2 && config.OAuth2Config != nil {
...
func (s *SecureTransferHandler) initializeEncryptionKey() error {
	...
	case sink.EncryptionAlgorithmAES256:
		s.encryptionKey = make([]byte, 32) // 256 bits
	case sink.EncryptionAlgorithmAES128:
...
func (s *SecureTransferHandler) AuthenticateOutput(targetID string) error {
	...
	case sink.AuthenticationMethodOAuth2:
		return s.authenticateOAuth2()
	case sink.AuthenticationMethodClientCertificate:
		return s.authenticateClientCertificate()
	case sink.AuthenticationMethodUsernamePassword:
		return s.authenticateUsernamePassword()
	case sink.AuthenticationMethodAPIKey:
...
func (s *SecureTransferHandler) EncryptData(data []byte) ([]byte, error) {
	...
	switch s.config.EncryptionAlgorithm {
	case sink.EncryptionAlgorithmAES256, sink.EncryptionAlgorithmAES128:
		encryptedData, err = s.encryptAES(data)
	case sink.EncryptionAlgorithmRSA:
		encryptedData, err = s.encryptRSA(data)
	case sink.EncryptionAlgorithmChaCha20:
...
func (s *SecureTransferHandler) CreateSecureConnection(host, port string) (SecureConnection, error) {
	...
	case sink.SecurityProtocolTLS, sink.SecurityProtocolHTTPS:
		return s.createTLSConnection(host, port)
	case sink.SecurityProtocolSFTP:
		return s.createSFTPConnection(host, port)
	case sink.SecurityProtocolSSL:
