...
type ErrorRoutingStrategy interface {
	Route(errorItem *ErrorItem) ErrorRoutingStrategyType
	GetName() string
}
...
// Initialize 初始化错误处理器
func (e *SinkErrorHandler) Initialize(config *ErrorHandlingConfig) error {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *SinkErrorHandler) determineRoutingStrategy(errorItem *ErrorItem) ErrorRoutingStrategyType {
	// 检查是否为临时错误
	if e.isTransientError(errorItem.Error) {
		if errorItem.RetryCount < e.config.MaxRetryCount {
			return ErrorRoutingStrategyRetry
		} else {
			return ErrorRoutingStrategyDeadLetter
		}
	}
...
func (e *SinkErrorHandler) isTransientError(err error) bool {
	// 检查是否为临时输出错误
	if _, ok := err.(*TransientOutputError); ok {
		return true
	}
...
// routeError 路由错误
func (e *SinkErrorHandler) routeError(errorItem *ErrorItem, strategy ErrorRoutingStrategyType) {
	switch strategy {
...
func (e *SinkErrorHandler) routeToRetry(errorItem *ErrorItem) {
	...
	errorItem.FlowFile.Attributes["retry.count"] = fmt.Sprintf("%d", errorItem.RetryCount)
...
func (e *SinkErrorHandler) getRetryCount(flowFile *flowfile.FlowFile) int {
	retryCountStr := flowFile.Attributes["retry.count"]
	if retryCountStr == "" {
		return 0
	}
...
// Route 默认错误路由策略
func (s *DefaultErrorRoutingStrategy) Route(errorItem *ErrorItem) ErrorRoutingStrategyType {
...
// Route 重试错误路由策略
func (s *RetryErrorRoutingStrategy) Route(errorItem *ErrorItem) ErrorRoutingStrategyType {
...
// Route 丢弃错误路由策略
func (s *DropErrorRoutingStrategy) Route(errorItem *ErrorItem) ErrorRoutingStrategyType {
	return ErrorRoutingStrategyDrop
}
...
// Route 死信错误路由策略
func (s *DeadLetterErrorRoutingStrategy) Route(errorItem *ErrorItem) ErrorRoutingStrategyType {
	return ErrorRoutingStrategyDeadLetter
}
