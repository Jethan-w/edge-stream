...
type TimestampExtractor interface {
	...
	ExtractTimestamp(flowFile *flowfile.FlowFile) int64
...
// ExtractTimestamp 从 FlowFile 提取事件时间戳
func (jte *JsonTimestampExtractor) ExtractTimestamp(flowFile *flowfile.FlowFile) int64 {
...
func (ate *AttributeTimestampExtractor) ExtractTimestamp(flowFile *flowfile.FlowFile) int64 {
	timestampStr := flowFile.Attributes[ate.timestampAttribute]
	if timestampStr == "" {
		// 属性不存在，返回当前时间
		return time.Now().UnixMilli()
	}
...
func (pte *ProcessingTimeExtractor) ExtractTimestamp(flowFile *flowfile.FlowFile) int64 {
	// 直接返回当前处理时间
	return time.Now().UnixMilli()
}
...
func (cte *CsvTimestampExtractor) ExtractTimestamp(flowFile *flowfile.FlowFile) int64 {
...
type LateDataStrategy struct {
	...
	sideOutputQueue chan *flowfile.FlowFile
}
...
func NewLateDataStrategy(maxLateness int64, handling LateDataHandling) *LateDataStrategy {
	return &LateDataStrategy{
		...
		handling:        handling,
		sideOutputQueue: make(chan *flowfile.FlowFile, 1000),
	}
}
...
func (lds *LateDataStrategy) HandleLateData(flowFile *flowfile.FlowFile, eventTime, watermark int64) bool {
...
func (lds *LateDataStrategy) GetSideOutputQueue() <-chan *flowfile.FlowFile {
