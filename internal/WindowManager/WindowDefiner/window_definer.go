...
type TimeWindow struct {
	*windowmanager.AbstractWindow
	startTime int64
	endTime   int64
}

// NewTimeWindow 创建时间窗口
func NewTimeWindow(startTime, endTime int64, stateManager windowmanager.WindowStateManager) *TimeWindow {
	abstractWindow := windowmanager.NewAbstractWindow(windowmanager.WindowTypeTumbling, stateManager)
...
func NewCountWindow(maxCount int, stateManager windowmanager.WindowStateManager) *CountWindow {
	abstractWindow := windowmanager.NewAbstractWindow(windowmanager.WindowTypeCountBased, stateManager)
...
func NewSessionWindow(sessionTimeout time.Duration, stateManager windowmanager.WindowStateManager) *SessionWindow {
	abstractWindow := windowmanager.NewAbstractWindow(windowmanager.WindowTypeSession, stateManager)
...
func (sw *SessionWindow) AddFlowFile(flowFile *windowmanager.FlowFile) error {
	sw.mu.Lock()
...
func NewSlidingWindow(windowSize, slideInterval int64, stateManager windowmanager.WindowStateManager) *SlidingWindow {
	now := time.Now().UnixMilli()
	abstractWindow := windowmanager.NewAbstractWindow(windowmanager.WindowTypeSliding, stateManager)
...
// CreateWindow 创建窗口
func (wf *WindowFactory) CreateWindow(windowType windowmanager.WindowType, config WindowConfig, stateManager windowmanager.WindowStateManager) (windowmanager.Window, error) {
	switch windowType {
	case windowmanager.WindowTypeTumbling:
		return wf.createTumblingWindow(config, stateManager)
	case windowmanager.WindowTypeSliding:
		return wf.createSlidingWindow(config, stateManager)
	case windowmanager.WindowTypeSession:
		return wf.createSessionWindow(config, stateManager)
	case windowmanager.WindowTypeCountBased:
...
func (wf *WindowFactory) createTumblingWindow(config WindowConfig, stateManager windowmanager.WindowStateManager) (windowmanager.Window, error) {
...
func (wf *WindowFactory) createSlidingWindow(config WindowConfig, stateManager windowmanager.WindowStateManager) (windowmanager.Window, error) {
...
func (wf *WindowFactory) createSessionWindow(config WindowConfig, stateManager windowmanager.WindowStateManager) (windowmanager.Window, error) {
...
func (wf *WindowFactory) createCountWindow(config WindowConfig, stateManager windowmanager.WindowStateManager) (windowmanager.Window, error) {
...
func (wts *WindowTriggerStrategy) ShouldTriggerByTime(window windowmanager.Window, currentTime int64) bool {
	return currentTime >= window.GetEndTime()
}

// ShouldTriggerByCount 基于数量的触发策略
func (wts *WindowTriggerStrategy) ShouldTriggerByCount(window windowmanager.Window, threshold int) bool {
	return len(window.GetFlowFiles()) >= threshold
}

// ShouldTrigger 混合触发策略
func (wts *WindowTriggerStrategy) ShouldTrigger(window windowmanager.Window, currentTime int64, countThreshold int) bool {
...
// ProcessWindow 处理窗口
func (wop *WindowOutputProcessor) ProcessWindow(window windowmanager.Window, aggregator windowmanager.WindowAggregator) error {
	wop.mu.Lock()
	defer wop.mu.Unlock()
...
func (wop *WindowOutputProcessor) createOutputFlowFile(result interface{}, window windowmanager.Window) *windowmanager.FlowFile {
	// 序列化结果
	...
	return windowmanager.NewFlowFile(
		fmt.Sprintf("output_%s", window.GetID()),
		resultData,
		attributes,
	)
}
