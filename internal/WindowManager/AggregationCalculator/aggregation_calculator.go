...
type WindowAggregator interface {
	...
	Aggregate(flowFile *windowmanager.FlowFile) error
	...
	GetAggregationFunction() windowmanager.AggregationFunction
}
...
type AggregationStrategy interface {
	...
	ComputeAggregations(flowFiles []*windowmanager.FlowFile) (map[string]interface{}, error)
...
// Aggregate 聚合单个 FlowFile
func (aa *AverageAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	aa.mu.Lock()
	defer aa.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (aa *AverageAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionAvg
}

// extractValue 提取数值
func (aa *AverageAggregator) extractValue(flowFile *windowmanager.FlowFile) (float64, error) {
	content := flowFile.GetContent()

	var jsonData map[string]interface{}
	if err := json.Unmarshal(content, &jsonData); err != nil {
		// 尝试从属性中获取
		if valueStr := flowFile.GetAttribute(aa.aggregationField); valueStr != "" {
...
// Aggregate 聚合单个 FlowFile
func (sa *SumAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	sa.mu.Lock()
	defer sa.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (sa *SumAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionSum
}

// extractValue 提取数值
func (sa *SumAggregator) extractValue(flowFile *windowmanager.FlowFile) (float64, error) {
	content := flowFile.GetContent()

	var jsonData map[string]interface{}
	if err := json.Unmarshal(content, &jsonData); err != nil {
		// 尝试从属性中获取
		if valueStr := flowFile.GetAttribute(sa.aggregationField); valueStr != "" {
			if value, err := strconv.ParseFloat(valueStr, 64); err == nil {
				return value, nil
			}
		}
...
// Aggregate 聚合单个 FlowFile
func (ma *MaxAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	ma.mu.Lock()
	defer ma.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (ma *MaxAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionMax
}

// extractValue 提取数值
func (ma *MaxAggregator) extractValue(flowFile *windowmanager.FlowFile) (float64, error) {
	content := flowFile.GetContent()

	var jsonData map[string]interface{}
	if err := json.Unmarshal(content, &jsonData); err != nil {
		// 尝试从属性中获取
		if valueStr := flowFile.GetAttribute(ma.aggregationField); valueStr != "" {
			if value, err := strconv.ParseFloat(valueStr, 64); err == nil {
				return value, nil
			}
		}
...
// Aggregate 聚合单个 FlowFile
func (ma *MinAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	ma.mu.Lock()
	defer ma.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (ma *MinAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionMin
}

// extractValue 提取数值
func (ma *MinAggregator) extractValue(flowFile *windowmanager.FlowFile) (float64, error) {
	content := flowFile.GetContent()
...
// Aggregate 聚合单个 FlowFile
func (ca *CountAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	ca.mu.Lock()
	defer ca.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (ca *CountAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionCount
}
...
// Aggregate 聚合单个 FlowFile
func (fa *FirstAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	fa.mu.Lock()
	defer fa.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (fa *FirstAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionFirst
}

// extractValue 提取数值
func (fa *FirstAggregator) extractValue(flowFile *windowmanager.FlowFile) (interface{}, error) {
	content := flowFile.GetContent()

	var jsonData map[string]interface{}
	if err := json.Unmarshal(content, &jsonData); err != nil {
		// 尝试从属性中获取
		if valueStr := flowFile.GetAttribute(fa.aggregationField); valueStr != "" {
			return valueStr, nil
		}
...
// Aggregate 聚合单个 FlowFile
func (la *LastAggregator) Aggregate(flowFile *windowmanager.FlowFile) error {
	la.mu.Lock()
	defer la.mu.Unlock()
...
// GetAggregationFunction 获取聚合函数类型
func (la *LastAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
	return windowmanager.AggregationFunctionLast
}

// extractValue 提取数值
func (la *LastAggregator) extractValue(flowFile *windowmanager.FlowFile) (interface{}, error) {
	content := flowFile.GetContent()

	var jsonData map[string]interface{}
	if err := json.Unmarshal(content, &jsonData); err != nil {
		// 尝试从属性中获取
		if valueStr := flowFile.GetAttribute(la.aggregationField); valueStr != "" {
			return valueStr, nil
		}
		return nil, fmt.Errorf("failed to parse JSON content")
	}
...
// ComputeAggregations 计算聚合结果
func (mas *MultiAggregationStrategy) ComputeAggregations(flowFiles []*windowmanager.FlowFile) (map[string]interface{}, error) {
	mas.mu.RLock()
...
func (af *AggregatorFactory) CreateAggregator(function windowmanager.AggregationFunction, field string) (WindowAggregator, error) {
	switch function {
	case windowmanager.AggregationFunctionCount:
		return NewCountAggregator(), nil
	case windowmanager.AggregationFunctionSum:
		return NewSumAggregator(field), nil
	case windowmanager.AggregationFunctionAvg:
		return NewAverageAggregator(field), nil
	case windowmanager.AggregationFunctionMax:
		return NewMaxAggregator(field), nil
	case windowmanager.AggregationFunctionMin:
		return NewMinAggregator(field), nil
	case windowmanager.AggregationFunctionFirst:
		return NewFirstAggregator(field), nil
	case windowmanager.AggregationFunctionLast:
