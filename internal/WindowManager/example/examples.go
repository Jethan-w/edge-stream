...
type TemperatureWindowAnalyzer struct {
	...
	timestampExtractor  windowmanager.TimestampExtractor
	aggregationStrategy *windowmanager.MultiAggregationStrategy
	outputProcessor     *windowmanager.WindowOutputProcessor
	trigger             *windowmanager.StandardWindowTrigger
}
...
func NewTemperatureWindowAnalyzer() *TemperatureWindowAnalyzer {
	...
	// 创建聚合策略
	aggregationStrategy := windowmanager.NewMultiAggregationStrategy()
	aggregationStrategy.AddAggregator(windowmanager.NewAverageAggregator("temperature"))
	aggregationStrategy.AddAggregator(windowmanager.NewMaxAggregator("temperature"))
	aggregationStrategy.AddAggregator(windowmanager.NewMinAggregator("temperature"))
	aggregationStrategy.AddAggregator(windowmanager.NewCountAggregator())

	// 创建输出处理器
	outputProcessor := windowmanager.NewWindowOutputProcessor()

	// 创建触发器
	trigger := windowmanager.NewStandardWindowTrigger()
	trigger.OutputProcessor = (outputProcessor)
	trigger.Aggregator = (aggregationStrategy)
...
func CountWindowExample() error {
	...
	// 创建聚合策略
	aggregationStrategy := windowmanager.NewMultiAggregationStrategy()
	aggregationStrategy.AddAggregator(windowmanager.NewSumAggregator("value"))
	aggregationStrategy.AddAggregator(windowmanager.NewAverageAggregator("value"))
...
func SessionWindowExample() error {
	...
	aggregationStrategy := windowmanager.NewMultiAggregationStrategy()
	aggregationStrategy.AddAggregator(windowmanager.NewCountAggregator())
	aggregationStrategy.AddAggregator(windowmanager.NewFirstAggregator("user_id"))
	aggregationStrategy.AddAggregator(windowmanager.NewLastAggregator("user_id"))
...
func SlidingWindowExample() error {
	// 创建窗口工厂
	factory := windowmanager.NewWindowFactory()

	// 创建滑动窗口配置
	config := windowmanager.WindowConfig{
		WindowSize:    300000, // 5分钟窗口
		SlideInterval: 60000,  // 1分钟滑动间隔
	}

	// 创建状态管理器
	stateManager := windowmanager.NewMemoryStateManager()

	// 创建滑动窗口
	window, err := factory.CreateWindow(windowmanager.WindowTypeSliding, config, stateManager)
	if err != nil {
	...
	aggregationStrategy := windowmanager.NewMultiAggregationStrategy()
	aggregationStrategy.AddAggregator(windowmanager.NewAverageAggregator("value"))
	aggregationStrategy.AddAggregator(windowmanager.NewMaxAggregator("value"))
...
func LateDataHandlingExample() error {
	...
	// 创建延迟数据处理策略
	lateDataStrategy := windowmanager.NewLateDataStrategy(
		300000, // 5分钟最大延迟
		windowmanager.LateDataHandlingSideOutput,
	)

	// 创建水印管理器
	watermarkManager := windowmanager.NewSimpleWatermarkManager()

	// 创建水印触发策略
	// triggerStrategy := windowmanager.NewWatermarkTriggerStrategy(watermarkManager, 300000)
...
func (cta *CustomTemperatureAggregator) Aggregate(flowFile *flowfile.FlowFile) error {
	cta.mu.Lock()
	defer cta.mu.Unlock()
...
func (cta *CustomTemperatureAggregator) GetAggregationFunction() windowmanager.AggregationFunction {
