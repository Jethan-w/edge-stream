...
// ReceiveVideo 接收视频数据
func (mmr *MultiModalReceiver) ReceiveVideo(packet *source.VideoPacket) error {
	mmr.mu.Lock()
	defer mmr.mu.Unlock()
...
// ReceiveText 接收文本数据
func (mmr *MultiModalReceiver) ReceiveText(packet *source.TextPacket) error {
	mmr.mu.Lock()
	defer mmr.mu.Unlock()
...
func (mmr *MultiModalReceiver) ReceiveBinary(packet *source.BinaryPacket) error {
	mmr.mu.Lock()
	defer mmr.mu.Unlock()
...
func (mmr *MultiModalReceiver) ReceiveCustom(packet *source.CustomDataPacket) error {
	mmr.mu.Lock()
	defer mmr.mu.Unlock()
...
// Receive 接收视频数据
func (vr *VideoReceiver) Receive(packet *source.VideoPacket) error {
	vr.mu.Lock()
	defer vr.mu.Unlock()
...
func (vr *VideoReceiver) validateVideoPacket(packet *source.VideoPacket) error {
	if packet == nil {
...
func (vr *VideoReceiver) processVideoPacket(packet *source.VideoPacket) error {
	// 这里实现具体的视频处理逻辑
	// 例如：解码、压缩、格式转换等
...
// Receive 接收文本数据
func (tr *TextReceiver) Receive(packet *source.TextPacket) error {
	tr.mu.Lock()
	defer tr.mu.Unlock()
...
func (tr *TextReceiver) validateTextPacket(packet *source.TextPacket) error {
	if packet == nil {
...
func (tr *TextReceiver) processTextPacket(packet *source.TextPacket) error {
	// 这里实现具体的文本处理逻辑
	// 例如：编码转换、格式验证、内容过滤等
...
// Receive 接收二进制数据
func (br *BinaryReceiver) Receive(packet *source.BinaryPacket) error {
	br.mu.Lock()
	defer br.mu.Unlock()
...
func (br *BinaryReceiver) validateBinaryPacket(packet *source.BinaryPacket) error {
	if packet == nil {
...
func (br *BinaryReceiver) processBinaryPacket(packet *source.BinaryPacket) error {
	// 这里实现具体的二进制处理逻辑
	// 例如：数据校验、格式转换、压缩等
...
// Receive 接收自定义数据
func (cr *CustomReceiver) Receive(packet *source.CustomDataPacket) error {
	cr.mu.Lock()
	defer cr.mu.Unlock()
...
func (cr *CustomReceiver) validateCustomPacket(packet *source.CustomDataPacket) error {
	if packet == nil {
...
func (cr *CustomReceiver) processCustomPacket(packet *source.CustomDataPacket) error {
	// 这里实现具体的自定义数据处理逻辑
	// 根据数据类型进行不同的处理
