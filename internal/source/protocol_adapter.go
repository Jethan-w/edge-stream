...
func (apa *AbstractProtocolAdapter) Receive() (DataPacket, error) {
	// TODO implement me
	panic("implement me")
}

func (apa *AbstractProtocolAdapter) Close() error {
	// TODO implement me
	panic("implement me")
}
...
// Receive 接收数据
func (ta *TCPAdapter) Receive() (DataPacket, error) {
	// 接受连接
...
// Receive 接收数据
func (ua *UDPAdapter) Receive() (DataPacket, error) {
	// 读取UDP数据
	buffer := make([]byte, 1024)
...
// Receive 接收数据
func (ha *HTTPAdapter) Receive() (DataPacket, error) {
	// HTTP适配器通过回调接收数据
...
func (ma *ModbusAdapter) Initialize(config map[string]string) error {
	...
	_, err := strconv.Atoi(baudRateStr)
	if err != nil {
...
// Receive 接收数据
func (ma *ModbusAdapter) Receive() (DataPacket, error) {
	// 这里实现Modbus数据读取逻辑
	// 例如：读取传感器寄存器数据

	// 示例：创建模拟的Modbus数据包
	packet := &CustomDataPacket{
