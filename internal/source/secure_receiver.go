...
func (sr *SecureReceiver) createTLSConfig(context *TLSContext) (*tls.Config, error) {
	// 加载证书和私钥
...
func (am *AuthenticationManager) ValidateOAuth2Token(token *OAuth2Token) error {
	am.mu.Lock()
	defer am.mu.Unlock()
...
// ValidateToken 验证令牌
func (os *OAuth2Service) ValidateToken(token *OAuth2Token) error {
	os.mu.RLock()
	defer os.mu.RUnlock()
...
// SetAuthToken 设置认证令牌
func (sc *SecureConnection) SetAuthToken(token *OAuth2Token) {
	sc.mu.Lock()
	defer sc.mu.Unlock()
	sc.authToken = token
}
...
// GetAuthToken 获取认证令牌
func (sc *SecureConnection) GetAuthToken() *OAuth2Token {
	sc.mu.RLock()
	defer sc.mu.RUnlock()
	return sc.authToken
}
