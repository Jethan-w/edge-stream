...
type ConfigManager struct {
	configs         map[string]*source.SourceConfiguration
...
func NewConfigManager() *ConfigManager {
	return &ConfigManager{
		configs:         make(map[string]*source.SourceConfiguration),
...
func (cm *ConfigManager) GetConfiguration(sourceID string) (*source.SourceConfiguration, bool) {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
...
func (cm *ConfigManager) ValidateConfiguration(sourceID string) *source.ValidationResult {
	cm.mu.RLock()
	defer cm.mu.RUnlock()

	config, exists := cm.configs[sourceID]
	if !exists {
		result := &source.ValidationResult{Valid: false}
...
// Parse 解析配置
func (scp *SourceConfigParser) Parse(properties map[string]string) (*source.SourceConfiguration, error) {
	scp.mu.Lock()
	defer scp.mu.Unlock()
...
func (scp *SourceConfigParser) ParseFromFile(filePath string) (*source.SourceConfiguration, error) {
	// 读取文件内容
...
// Validate 验证数据源配置
func (dsv *DataSourceValidator) Validate(config *source.SourceConfiguration) *source.ValidationResult {
	dsv.mu.RLock()
	defer dsv.mu.RUnlock()

	result := &source.ValidationResult{Valid: true}
...
func (dsv *DataSourceValidator) checkConnectivity(config *source.SourceConfiguration) error {
	// 这里实现实际的连接性检查逻辑
...
func (dsv *DataSourceValidator) checkPermissions(config *source.SourceConfiguration) error {
	// 这里实现实际的权限检查逻辑
	// 例如：检查文件权限、网络权限等
...
func (dsv *DataSourceValidator) validateDataFormat(config *source.SourceConfiguration) error {
	// 这里实现实际的数据格式验证逻辑
	// 例如：检查数据格式配置、验证编码设置等
...
func (dsv *DataSourceValidator) checkConfigurationCompleteness(config *source.SourceConfiguration) error {
	// 检查必需配置项
	if config.Host == "" {
		return fmt.Errorf("主机地址不能为空")
	}
...
func (ccn *ConfigChangeNotifier) NotifyConfigChange(sourceID string, config *source.SourceConfiguration) {
	ccn.mu.RLock()
...
func (dsc *DynamicSourceConfigurator) notifyConfigurationChange(sourceID string, config *source.SourceConfiguration) {
	// 这里可以通知相关的组件配置已变更
