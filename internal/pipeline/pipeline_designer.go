...
func (pd *PipelineDesigner) CreateProcessGroup(name string) *ProcessGroup {
	pd.mu.Lock()
	defer pd.mu.Unlock()

	id := generateProcessGroupID()
	group := NewProcessGroup(id, name)

	// 设置默认配置
	config := NewProcessGroupConfiguration()
...
// AddProcessor 添加处理器
func (pd *PipelineDesigner) AddProcessor(group *ProcessGroup, processorConfig *ProcessorDTO) error {
	pd.mu.Lock()
	defer pd.mu.Unlock()
	...
	if processorConfig.Scheduling != nil {
		processorNode.Configuration.Scheduling = &SchedulingConfiguration{
...
func (pd *PipelineDesigner) CreateConnection(group *ProcessGroup, connectionConfig *ConnectionDTO) error {
	pd.mu.Lock()
	defer pd.mu.Unlock()
...
func (pd *PipelineDesigner) createPrioritizer(config *PrioritizerConfig) (QueuePrioritizer, error) {
	switch config.Type {
	case "oldest_first":
		return NewOldestFirstPrioritizer(), nil
	case "newest_first":
		return NewNewestFirstPrioritizer(), nil
	default:
...
func (pd *PipelineDesigner) GetProcessGroup(id string) (*ProcessGroup, bool) {
	pd.mu.RLock()
	defer pd.mu.RUnlock()
...
func (lp *LogProcessor) Stop(ctx processor.ProcessContext) error {
	// TODO implement me
	panic("implement me")
}

func (lp *LogProcessor) Start(ctx processor.ProcessContext) error {
	// TODO implement me
	panic("implement me")
}
...
// 	return &LogProcessor{
// 		AbstractProcessor: *processor.NewRouteProcessor(),
// 	}
// }

// Process 处理数据
func (lp *LogProcessor) Process(ctx processor.ProcessContext, flowfile *flowfile.FlowFile) (*flowfile.FlowFile, error) {
	// 简单的日志处理逻辑
...
type PipelineUIDesigner interface {
	...
	CreateConnection(source, destination *ProcessorNode) error
	ConfigureProcessorProperties(processor *ProcessorNode, properties map[string]string) error
	MoveProcessor(processor *ProcessorNode, position *PositionDTO) error
	DeleteProcessor(processor *ProcessorNode) error
	DeleteConnection(connection *Connection) error
}
...
// CreateConnection 创建连接
func (spud *StandardPipelineUIDesigner) CreateConnection(source, destination *ProcessorNode) error {
	spud.mu.Lock()
	defer spud.mu.Unlock()
...
func (spud *StandardPipelineUIDesigner) ConfigureProcessorProperties(processor *ProcessorNode, properties map[string]string) error {
	spud.mu.Lock()
	defer spud.mu.Unlock()
...
// MoveProcessor 移动处理器
func (spud *StandardPipelineUIDesigner) MoveProcessor(processor *ProcessorNode, position *PositionDTO) error {
	spud.mu.Lock()
	defer spud.mu.Unlock()
...
func (spud *StandardPipelineUIDesigner) DeleteProcessor(processor *ProcessorNode) error {
	spud.mu.Lock()
	defer spud.mu.Unlock()
...
func (spud *StandardPipelineUIDesigner) DeleteConnection(connection *Connection) error {
	spud.mu.Lock()
	defer spud.mu.Unlock()
...
// DeployPipeline 部署流程
func (po *PipelineOrchestrator) DeployPipeline(group *ProcessGroup) error {
	po.mu.Lock()
	defer po.mu.Unlock()
...
// StartPipeline 启动流程
func (po *PipelineOrchestrator) StartPipeline(group *ProcessGroup) error {
	po.mu.Lock()
	defer po.mu.Unlock()
...
// StopPipeline 停止流程
func (po *PipelineOrchestrator) StopPipeline(group *ProcessGroup) error {
	po.mu.Lock()
	defer po.mu.Unlock()
...
func (po *PipelineOrchestrator) validateProcessGroup(group *ProcessGroup) *ValidationResult {
...
func (pe *PipelineExecutor) DeployProcessGroup(group *ProcessGroup) error {
	pe.mu.Lock()
	defer pe.mu.Unlock()
...
func (pe *PipelineExecutor) StartProcessGroup(group *ProcessGroup) error {
	pe.mu.Lock()
	defer pe.mu.Unlock()
	...
	for _, processorID := range processors {
		process, exists := group.GetProcessor(processorID)
		if exists {
			c := processor.ProcessContext{
				Context: context.Background(),
			}
			if err := process.Start(c); err != nil {
...
func (pe *PipelineExecutor) StopProcessGroup(group *ProcessGroup) error {
	pe.mu.Lock()
	defer pe.mu.Unlock()
