...
// SaveVersion 保存版本
func (vm *VersionManager) SaveVersion(group *pipeline.ProcessGroup, comment string) error {
	vm.mu.Lock()
	defer vm.mu.Unlock()
...
func (vm *VersionManager) serializeProcessGroup(group *pipeline.ProcessGroup) (*VersionedFlow, error) {
	// 创建版本化流程
	...
	for _, childGroupID := range childGroups {
		...
		if exists {
			...
			versionedFlow.ChildGroups = append(versionedFlow.ChildGroups, versionedChildGroup)
		}
	}
