...
// RecordEvent 记录溯源事件
func (pt *ProvenanceTracker) RecordEvent(flowFile *flowfile.FlowFile, eventType ProvenanceEventType, processorID string, details map[string]string) error {
	pt.mu.Lock()
	defer pt.mu.Unlock()
...
func (dlt *DataLineageTracker) Trace(flowFile *flowfile.FlowFile) (*DataLineage, error) {
	dlt.mu.RLock()
