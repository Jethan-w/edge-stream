...
// Initialize 初始化处理器
func (tse *TextSemanticExtractor) Initialize(ctx context.Context) error {
...
// Process 处理数据
func (tse *TextSemanticExtractor) Process(ctx processor.ProcessContext, flowFile *processor.FlowFile) (*processor.FlowFile, error) {
	tse.SetState(processor.StateRunning)
	defer tse.SetState(processor.StateReady)
...
// Process 处理数据
func (sdse *StructuredDataSemanticExtractor) Process(ctx processor.ProcessContext, flowFile *processor.FlowFile) (*processor.FlowFile, error) {
	sdse.SetState(processor.StateRunning)
	defer sdse.SetState(processor.StateReady)
...
// Process 处理数据
func (ase *AdvancedSemanticExtractor) Process(ctx processor.ProcessContext, flowFile *processor.FlowFile) (*processor.FlowFile, error) {
	ase.SetState(processor.StateRunning)
	defer ase.SetState(processor.StateReady)
...
func (ase *AdvancedSemanticExtractor) hybridExtraction(ctx processor.ProcessContext, flowFile *processor.FlowFile) (*processor.FlowFile, error) {
	// 首先尝试结构化提取
