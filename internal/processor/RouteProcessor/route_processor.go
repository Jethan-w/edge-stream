...
// Initialize 初始化处理器
func (rp *RouteProcessor) Initialize(ctx context.Context) error {
...
// Process 处理数据
func (rp *RouteProcessor) Process(ctx processor.ProcessContext, flowFile *processor.FlowFile) (*processor.FlowFile, error) {
	rp.SetState(processor.StateRunning)
	defer rp.SetState(processor.StateReady)
...
func (rp *RouteProcessor) routeFlowFile(flowFile *processor.FlowFile, properties map[string]string) (string, error) {
	// 首先尝试基于属性的路由
...
func (rp *RouteProcessor) evaluateContentBasedRouting(flowFile *processor.FlowFile, properties map[string]string) string {
...
func (rp *RouteProcessor) evaluateMultiDimensionalRouting(flowFile *processor.FlowFile, properties map[string]string) string {
	// 评估高价值标准
...
func (rp *RouteProcessor) evaluateHighValueCriteria(flowFile *processor.FlowFile, properties map[string]string) bool {
	// 检查文件大小
...
func (rp *RouteProcessor) evaluateUrgencyCriteria(flowFile *processor.FlowFile, properties map[string]string) bool {
	// 检查时间戳
...
// Evaluate 评估路由规则
func (rr *RouteRule) Evaluate(flowFile *processor.FlowFile) bool {
...
type AdvancedRouteProcessor struct {
	*RouteProcessor
	priorityQueues map[string]chan *processor.FlowFile
}
...
func NewAdvancedRouteProcessor() *AdvancedRouteProcessor {
	arp := &AdvancedRouteProcessor{
		...
		priorityQueues: make(map[string]chan *processor.FlowFile),
	}

	// 初始化优先级队列
	arp.priorityQueues["high_priority"] = make(chan *processor.FlowFile, 1000)
	arp.priorityQueues["standard_priority"] = make(chan *processor.FlowFile, 1000)
	arp.priorityQueues["low_priority"] = make(chan *processor.FlowFile, 1000)
...
// Process 处理数据
func (arp *AdvancedRouteProcessor) Process(ctx processor.ProcessContext, flowFile *processor.FlowFile) (*processor.FlowFile, error) {
	arp.SetState(processor.StateRunning)
	defer arp.SetState(processor.StateReady)
...
func (arp *AdvancedRouteProcessor) routeByMultipleCriteria(flowFile *processor.FlowFile, properties map[string]string) string {
	// 评估高价值标准
...
func (arp *AdvancedRouteProcessor) GetPriorityQueue(queueName string) (chan *processor.FlowFile, bool) {
