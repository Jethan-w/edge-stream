...
func (sp *ScriptProcessor) Stop(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}

func (sp *ScriptProcessor) Start(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}
...
// Initialize 初始化处理器
func (sp *ScriptProcessor) Initialize(ctx ProcessContext) error {
...
// Process 处理数据
func (sp *ScriptProcessor) Process(ctx ProcessContext, flowFile *flowfile.FlowFile) (*flowfile.FlowFile, error) {
	sp.SetState(StateRunning)
	defer sp.SetState(StateReady)
...
// executeScript 执行脚本
func (sp *ScriptProcessor) executeScript(script CompiledScript, scriptExecutionContext *ScriptExecutionContext, properties map[string]string) (interface{}, error) {
	// 设置执行超时
	...
	ctx, cancel := context.WithTimeout(scriptExecutionContext.Background(), timeout)
	defer cancel()
	...
	go func() {
		result, err := sp.scriptEngine.Execute(script, scriptExecutionContext)
		if err != nil {
			errChan <- err
		} else {
			resultChan <- result
...
func (sp *ScriptProcessor) handleScriptResult(result interface{}, flowFile *flowfile.FlowFile, properties map[string]string) (*flowfile.FlowFile, error) {
	if result == nil {
		return flowFile, nil
	}
...
// Background 返回后台上下文
func (sec *ScriptExecutionContext) Background() ProcessContext {
...
func (cp *CustomProcessor) Stop(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}

func (cp *CustomProcessor) Start(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}
...
// Process 处理数据
func (cp *CustomProcessor) Process(ctx ProcessContext, flowFile *flowfile.FlowFile) (*flowfile.FlowFile, error) {
	cp.SetState(StateRunning)
	defer cp.SetState(StateReady)
...
func (dp *DynamicProcessor) Stop(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}

func (dp *DynamicProcessor) Start(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}
...
// Process 处理数据
func (dp *DynamicProcessor) Process(ctx ProcessContext, file *flowfile.FlowFile) (*flowfile.FlowFile, error) {
	dp.SetState(StateRunning)
	defer dp.SetState(StateReady)
	...
	// 处理结果
	if resultFlowFile, ok := result.(*flowfile.FlowFile); ok {
	...
	newFlowFile := &flowfile.FlowFile{
		UUID:       fmt.Sprintf("dynamic_%d", time.Now().UnixNano()),
