...
func (eh *ErrorHandler) Stop(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}

func (eh *ErrorHandler) Start(ctx ProcessContext) error {
	// TODO implement me
	panic("implement me")
}
...
// Initialize 初始化处理器
func (eh *ErrorHandler) Initialize(ctx ProcessContext) error {
...
// Process 处理数据
func (eh *ErrorHandler) Process(ctx ProcessContext, flowFile *flowfile.FlowFile) (*flowfile.FlowFile, error) {
	eh.SetState(StateRunning)
	defer eh.SetState(StateReady)
...
// handleError 处理错误
func (eh *ErrorHandler) handleError(flowFile *flowfile.FlowFile, errorMsg string, properties map[string]string) (*flowfile.FlowFile, error) {
	// 分析错误类型
...
// handleRetry 处理重试
func (eh *ErrorHandler) handleRetry(flowFile *flowfile.FlowFile, retryCount int, properties map[string]string) (*flowfile.FlowFile, error) {
	// 检查是否应该重试
...
func (eh *ErrorHandler) applyErrorRule(flowFile *flowfile.FlowFile, rule ErrorRule, errorMsg string) (*flowfile.FlowFile, error) {
	// 添加错误处理相关属性
...
func (eh *ErrorHandler) applyDefaultErrorStrategy(flowFile *flowfile.FlowFile, errorType ErrorType, errorMsg string, properties map[string]string) (*flowfile.FlowFile, error) {
...
func (eh *ErrorHandler) getRetryCount(flowFile *flowfile.FlowFile) int {
...
// Process 处理数据
func (ehp *ErrorHandlingProcessor) Process(ctx ProcessContext, flowFile *flowfile.FlowFile) (*flowfile.FlowFile, error) {
	ehp.SetState(StateRunning)
	defer ehp.SetState(StateReady)
...
func (ehp *ErrorHandlingProcessor) processFlowFile(flowFile *flowfile.FlowFile) error {
	// 这里实现具体的处理逻辑
	// 为了演示，随机生成错误
